...
[**exchange_rate**](UtilApi.md#exchange_rate) | **POST** /util/exchange_rate | convert between units
...
# **convert**
> ConvertResponse convert(convert_request=convert_request)
...
# **exchange_rate**
> InlineResponse200 exchange_rate(inline_object=inline_object)
...
with mainnet.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = mainnet.UtilApi(api_client)
    inline_object = mainnet.InlineObject() # InlineObject |  (optional)

    try:
        # convert between units
        api_response = api_instance.exchange_rate(inline_object=inline_object)
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling UtilApi->exchange_rate: %s\n" % e)
```
